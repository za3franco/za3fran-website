...
function buildPrompt(c) {
  ...
  var sections = fr ? [
    ...
    '3. TABLE DES MATIERES [1 page]: AVANT h2, <div class="section-number">Section 3</div>. h2 "Table des Matieres". Liste numerotee <ol> de 13 entrees (sections 2 a 14): pour chacune, format "Titre de section — description 8-12 mots du contenu". Densite cible: remplir la page (les descriptions etoffent la liste pour eviter blanc). Terminer IMPERATIVEMENT la section par <div class="page-break"></div> (la table des matieres occupe sa propre page).',
    ...
    +'(b) APRES le tableau concurrents, INSERER <div class="page-break"></div> (le tableau seul sur la 1re page de la section). PUIS h3 "Differenciation du Concept" + court chapeau (25-30 mots) PUIS une VISUALISATION: grille 2x2 de 4 cartes (chaque carte = <div> bordure gauche 4px cuivre #C9862A, fond #fff8f0, padding 1.25rem, border-radius 6px, marge 0.6rem). Chaque carte: grand numero cuivre (1 a 4) + titre axe en gras + 2-3 lignes (40-55 mots) comparant Zoco vs concurrents. Axes: (1) Vitesse + Authenticite, (2) Qualite tracee vs informel, (3) Experience coherente vs lenteur traditionnelle, (4) Premium accessible. Cartes spacieuses et aerees qui remplissent la 2e page de la section.',
    ...
    +'(c) h3 "Items Signature" + 3 plats sous forme <ul>. Pour chaque: nom en gras + description 25-30 mots (ingredients core, technique de preparation, prix indicatif). NE PAS inclure de section fournisseurs (traite dans Menu Engineer).\n'
    +'(d) h3 "Apercu Menu Engineering (Directionnel)" + court chapeau (25-30 mots) introduisant la matrice stars/plowhorses/puzzles/dogs PUIS petit tableau 4 lignes [Categorie|Profil marge/popularite|Items Zoco indicatifs]: Stars (haute marge + populaire), Plowhorses (basse marge + populaire), Puzzles (haute marge + peu populaire), Dogs (basse marge + peu populaire). Juste assez pour remplir le bas de la page, pas plus.',
    ...
    +'(e) h3 "Top 3 KPIs Operationnels J+30" + tableau 3 lignes [KPI|Cible J+30|Methode de mesure] — Couverts/jour atteints, Temps service moyen, Satisfaction client (NPS). Colonne Methode <= 10 mots. TOUTE la section 9 (encadre + staffing + ratios + KPIs) DOIT tenir sur 1 SEULE page.',
    ...
    +'(b) h3 "Detail des 3 Risques Critiques" + 3 blocs <div class="risk-block"> UNIQUEMENT (les 3 risques au plus haut score). Chaque bloc TRES CONCIS (le tableau recap + Risque 1 DOIVENT tenir ENSEMBLE sur la page 1): <h4>Risque N: Titre</h4> + ligne en gras "Probabilite: X | Impact: Y | Score: Z/10" + 1 paragraphe contexte court (25-30 mots MAX) + h5 "Mitigation & Contingence" + liste <ol> de 3 actions concretes (12-15 mots chacune MAX, 1 ligne) + 1 ligne "Plan B:" finale (12-15 mots MAX). APRES le 1er bloc (Risque 1), INSERER <div class="page-break"></div> (Risques 2-3 + autres risques sur page 2).\n'
    ...
    +'(c) AVANT le h3 Glossaire, INSERER <div class="page-break"></div> (scores + note methodo sur la 1re page de la section; glossaire + footer sur la derniere page). PUIS h3 "Glossaire" + tableau 10 lignes [Terme|Definition courte 12-15 mots]: Fast-Casual, Ghost Kitchen, Food Cost %, EBITDA, Point Mort, Couvert (CV), Ticket Moyen, ROI, Ramp-up, NPS.\n'
    ...
    '3. TABLE OF CONTENTS [1 page]: BEFORE h2, <div class="section-number">Section 3</div>. h2 "Table of Contents". Numbered <ol> of 13 entries (sections 2 to 14): format each "Section title — 8-12 word description". Density target: fill the page. End IMPERATIVELY with <div class="page-break"></div> (table of contents occupies its own page).',
    ...
    +'(b) AFTER the competitors table, INSERT <div class="page-break"></div> (table alone on the section first page). THEN h3 "Concept Differentiation" + short lead-in (25-30 words) THEN a VISUALISATION: a 2x2 grid of 4 cards (each card = <div> with 4px copper #C9862A left border, #fff8f0 background, 1.25rem padding, 6px border-radius, 0.6rem margin). Each card: large copper number (1 to 4) + bold axis title + 2-3 lines (40-55 words) comparing concept vs competitors. Axes: (1) Speed + Authenticity, (2) Traced quality vs informal, (3) Coherent experience vs traditional slowness, (4) Accessible premium. Spacious airy cards that fill the section second page.',
    ...
    +'(c) h3 "Signature Items" + 3 dishes as <ul>: bold name + 25-30 word description (ingredients, technique, indicative price). DO NOT include a suppliers section (handled in Menu Engineer).\n'
    +'(d) h3 "Menu Engineering Preview (Directional)" + short lead-in (25-30 words) introducing the stars/plowhorses/puzzles/dogs matrix THEN a small 4-row table [Category|Margin/popularity profile|Indicative Zoco items]: Stars (high margin + popular), Plowhorses (low margin + popular), Puzzles (high margin + low popularity), Dogs (low margin + low popularity). Just enough to fill the bottom of the page, no more.',
    ...
    +'(e) h3 "Top 3 Operational KPIs D+30" + 3-row table (Method column <= 10 words). The WHOLE of section 9 (box + staffing + ratios + KPIs) MUST fit on a single page.',
    ...
    +'FIVE sub-sections, EACH in <div class="estimate-box"> with <h4>Title</h4> + 25-35 word description, THEN its table. EXACT ORDER AND LAYOUT: 10A alone on page 1; 10B+10C together on page 2; 10D(P&L)+10E together on page 3.\n\n'
    +'10A. STARTUP BUDGET — box + 12-row table [Item|Low '+sym+'|High '+sym+'|Notes]: Fit-out, Kitchen equipment, Furniture/decor, IT/POS, Licenses, Legal fees, Working capital (3 months), Pre-opening marketing, Cash reserve (3 months min), Contingency 8%, TOTAL, Gap vs declared budget '+(c.snap.budget||'N/A')+' '+cur+'. AFTER this 10A table, INSERT <div class="page-break"></div>.\n\n'
    ...
    +'10C. BREAK-EVEN SENSITIVITY — box + 4-row table [Scenario|Ticket -10%|Ticket Base|Ticket +15%]: Covers -20%, Covers Base, Covers +30%. Each cell = monthly break-even '+sym+'. One synthesis line below the table. AFTER that line, INSERT <div class="page-break"></div>.\n\n'
    ...
    +'10E. ROI & FUNDING — box + TWO tables: (1) ROI Parameters 5-row; (2) Financing structure 3-row [Source|Amount '+sym+'|%|Notes]. DO NOT include a "Financing Institutions" table (reserved for Business Plan Pro). 10D and 10E must fit together on page 3.',
    ...
    +'(b) h3 "Detail of 3 Critical Risks" + 3 <div class="risk-block"> ONLY (the 3 highest-scored risks). Each VERY COMPACT block (the summary table + Risk 1 MUST fit TOGETHER on page 1): <h4>Risk N: Title</h4> + bold line "Probability: X | Impact: Y | Score: Z/10" + short 25-30 word context paragraph MAX + h5 "Mitigation & Contingency" + <ol> of 3 numbered actions (12-15 words each MAX, 1 line) + final "Plan B:" line (12-15 words MAX). AFTER the 1st block (Risk 1), INSERT <div class="page-break"></div> (Risks 2-3 + other risks on page 2).\n'
    ...
    +'(c) BEFORE the Glossary h3, INSERT <div class="page-break"></div> (scores + methodology note on the section first page; glossary + footer on the last page). THEN h3 "Glossary" + 10-row table [Term|Short definition 12-15 words].\n'
