...
function buildPrompt(c) {
  ...
  var sections = fr ? [
    ...
    +'(b) Tout le bloc differenciation dans un <div class="diff-block"> (il demarre AUTOMATIQUEMENT sur une nouvelle page, le tableau concurrents reste seul sur la 1re page): h3 "Differenciation du Concept" + court chapeau (25-30 mots) PUIS grille 2x2 de 4 cartes, chaque carte = <div class="diff-card" style="border-left:4px solid #C9862A;background:#fff8f0;padding:1.25rem;border-radius:6px;margin:0.6rem"> contenant grand numero cuivre (1 a 4) + titre axe en gras + 2-3 lignes (40-50 mots) comparant Zoco vs concurrents. Axes: (1) Vitesse + Authenticite, (2) Qualite tracee vs informel, (3) Experience coherente vs lenteur traditionnelle, (4) Premium accessible. Cartes spacieuses et aerees qui remplissent la 2e page.',
    ...
    +'(d) h3 "Apercu Menu Engineering (Directionnel)" + court chapeau (20-25 mots) introduisant la matrice stars/plowhorses/puzzles/dogs PUIS petit tableau COMPACT 4 lignes [Categorie|Profil marge/popularite|Items Zoco] (cellules courtes): Stars (haute marge + populaire), Plowhorses (basse marge + populaire), Puzzles (haute marge + peu populaire), Dogs (basse marge + peu populaire). TOUTE la section 8 (encadre + structure + signatures + apercu) DOIT tenir sur 1 SEULE page.',
    ...
    +'10C. ANALYSE SENSIBILITE POINT MORT — encadre <div class="estimate-box" style="margin-top:3rem"> (marge haute pour aerer la page 10B+10C) + tableau 4 lignes (en-tete + 3 lignes scenarios) [Scenario|Ticket -10%|Ticket Base|Ticket +15%]: Couverts -20%, Couverts Base, Couverts +30%. Chaque cellule = point mort mensuel en '+sym+'. Sous le tableau, 1 ligne synthese: "Point mort conservateur: X '+sym+'/mois = Y couverts/jour. Atteint en QZ A1." APRES cette ligne synthese, INSERER <div class="page-break"></div>.\n\n'
    ...
    +'(d) h3 "Top 3 KPIs Marketing" + tableau 3 lignes [KPI|Cible J+30|Methode]: Followers reseaux, Base email engagee, Clients repeat. Colonne Methode <= 10 mots. TOUTE la section 11 (encadre + calendrier + mix canaux + KPIs) DOIT tenir sur 1 SEULE page.',
    ...
    +'10C. BREAK-EVEN SENSITIVITY — box <div class="estimate-box" style="margin-top:3rem"> (extra top margin to spread out the 10B+10C page) + 4-row table [Scenario|Ticket -10%|Ticket Base|Ticket +15%]: Covers -20%, Covers Base, Covers +30%. Each cell = monthly break-even '+sym+'. One synthesis line below the table. AFTER that line, INSERT <div class="page-break"></div>.\n\n'
    ...
    +'(d) h3 "Top 3 Marketing KPIs" + 3-row table (Method column <= 10 words). The WHOLE of section 11 (box + timeline + channel mix + KPIs) MUST fit on a single page.',
    ...
    +'(b) h3 "Detail of 3 Critical Risks" (render this h3 with style="margin-top:2.5rem" for breathing room between the table and Risk 1) + 3 <div class="risk-block"> ONLY (the 3 highest-scored risks). Each VERY COMPACT block (the summary table + Risk 1 MUST fit TOGETHER on page 1): <h4>Risk N: Title</h4> + bold line "Probability: X | Impact: Y | Score: Z/10" + short 25-30 word context paragraph MAX + h5 "Mitigation & Contingency" + <ol> of 3 numbered actions (12-15 words each MAX, 1 line) + final "Plan B:" line (12-15 words MAX). AFTER the 1st block (Risk 1), INSERT <div class="page-break"></div> (Risks 2-3 + other risks on page 2).\n'
