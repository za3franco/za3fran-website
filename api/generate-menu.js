...
3. MENU ENGINEERING MATRIX TABLE (Section 4): keep any intro text before the table to one short sentence at most — a long paragraph here stalls the heading on one page while the table jumps to the next, leaving a large blank gap. Never put the rationale text in a narrow table column either — a narrow column forces heavy word-wrapping and makes every row very tall. Instead, structure each item as TWO stacked table rows: Row A has the compact columns (Item | Section | Price | Food Cost % | Gross Margin | Classification badge); Row B is a single cell with colspan spanning the full table width, containing the rationale in smaller muted text (e.g. font-size:12px; color:#888880; padding:2px 8px 10px;) directly beneath Row A, with no visible border between A and B so they read as one entry. This keeps each item compact instead of tall.
...
5. THIS WEEK + CLOSING: these two must share a single page — put both inside the same <section> so there is no forced page-break between them. A separate page-breaking section for the short closing content wastes most of a page.
...
7. CLOSING: the closing CTA and print instruction must appear directly beneath the This Week box, in the SAME <section> as item 6 — do NOT start a new page-breaking section for the closing content. Combined, items 6 and 7 are short enough to share one page, and giving Closing its own forced page break would waste most of a page. Subtle CTA: "This report was generated by Za3fran's Menu Engineer. Visit za3fran.io or email hello@za3fran.io." Print instruction included.
