...
// GET  ?id=xxx&access_code=xxx  → validates code straight from the URL
//   (e.g. from the unified project dashboard or delivery email) and
//   serves the report directly, skipping the manual gate
...
// and back to the unified project dashboard.
//
// v2 changes:
//  - GET requests with a correct access_code query param now
//    authenticate directly (previously only POST or a cookie could
//    authenticate — a GET link with the code in the URL, like the
//    dashboard's "View" button or the delivery email link, would
//    always fall through to the manual gate even with a valid code).
//  - A run stuck at status 'generating' with no progress past this
//    function's own maxDuration (600s, see generate-menu.js) is now
//    treated as stale and safe to restart. generate-menu.js itself
//    has no guard against running twice concurrently, so this check
//    is deliberately conservative — it only retriggers once a
...
// Matches generate-menu.js's own maxDuration (600s) plus a buffer — a run
...
function injectToolbar(html, run, id) {
  const dashboardUrl = `${BASE_URL}/project.html?code=${encodeURIComponent(run.access_code)}`;
  const toolbar = `
...
export default async function handler(req, res) {
  ...
  const { data: run, error: runError } = await supabase
    ...
    .select('id, project_id, output_html, output_xlsx_url, access_code, status, currency, language, output_json')
    .eq('id', id)
    .maybeSingle();
  ...
  if (run.status === 'generating') {
    const startedAt = run.output_json?.generation_started_at;
    const startedMs = startedAt ? new Date(startedAt).getTime() : null;
    if (!startedMs || (Date.now() - startedMs) > STALE_GENERATION_MS) {
      console.log(`[report-menu-viewer] Run ${id} stuck at 'generating' past staleness threshold — restarting.`);
      shouldTrigger = true;
    }
  }
